  ...
  let stream = through(
    ...
    function () {
      ...
      if (nyc) {
        ...
        executeBin = nycBin
        if (_.isObject(nyc)) {
          const nycArgs = parseArgs(_.omit(nyc, ['bin']), true)
 ...
 */
function parseArgs (argsObj, isNyc) {
  let args = []
  _.each(argsObj, (val, key) => {
    if (_.isArray(val)) {
      _.each(val, (_val) => {
        addArg(args, key, _val, isNyc)
      })
    } else {
      addArg(args, key, val, isNyc)
    }
  })
  return args
}
